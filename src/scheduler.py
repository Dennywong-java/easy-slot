...
class VisaScheduler:
    ...
    def _handle_login_form(self):
        ...
        WebDriverWait(self.driver, 5).until(EC.presence_of_element_located((By.ID, "sign_in_form")))
        logging.info("Login form found")

        # Fill in email
        logging.info("Filling email field...")
        email_field = WebDriverWait(self.driver, 5).until(EC.presence_of_element_located((By.ID, "user_email")))
        email_field.clear()
        ...
        password_field = WebDriverWait(self.driver, 5).until(EC.presence_of_element_located((By.ID, "user_password")))
        ...
        sign_in_button = WebDriverWait(self.driver, 5).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "input.button.primary[name='commit']"))
        )
    ...
    def _handle_privacy_checkbox(self):
        ...
        try:
            ...
            if not checkbox.is_selected():
                checkbox_container = self.driver.find_element(By.CLASS_NAME, "icheckbox")
        ...
        try:
            ...
            for locator in success_indicators:
                try:
                    ...
                    login_success = True
                    logging.info(f"Successfully logged in - found indicator: {locator[1]}")
                    break
                except TimeoutException:
                    continue
    ...
    def _find_appointment_card(self):
        ...
        WebDriverWait(self.driver, 10).until(EC.presence_of_element_located((By.CLASS_NAME, "application")))
    ...
    def navigate_to_reschedule(self):
        ...
        try:
            ...
            if not target_card:
                ...
                raise Exception(f"Could not find appointment card with IVR: {self.config['appointment']['ivr_number']}")

            continue_button = target_card.find_element(By.CSS_SELECTOR, "a.button.primary.small")
    ...
    def _handle_reschedule_action(self):
        ...
        WebDriverWait(self.driver, 5).until(EC.presence_of_element_located((By.CLASS_NAME, "accordion")))

        accordion_items = self.driver.find_elements(By.CSS_SELECTOR, ".accordion-item a.accordion-title")
        for item in accordion_items:
            if "Reschedule Appointment" in item.text:
                item.click()
                logging.info("Clicked Reschedule Appointment accordion")

                content = item.find_element(By.XPATH, "following-sibling::div[contains(@class, 'accordion-content')]")
                reschedule_link = content.find_element(By.CSS_SELECTOR, "a.button.small.primary")
    ...
    def check_appointments(self):
        ...
        try:
            ...
            facilities = self.driver.find_element(By.ID, "appointments_consulate_appointment_facility_id")
            ...
            for facility in facility_options:
                ...
                if city_name in self.config["appointment"]["preferred_cities"]:
                    ...
                    try:
                        date_picker = self.driver.find_element(By.ID, "appointments_consulate_appointment_date")
                        if "disabled" not in date_picker.get_attribute("class"):
                            ...
                            if DateUtils.is_date_in_range(
                                ...
                                logging.info(f"Found available appointment in {city_name} on {available_date}")
                                self.notification.notify_appointment_found(city_name, available_date)
    ...
    def book_appointment(self, city, date):
        ...
        try:
            ...
            if time_options:
                ...
                success_message = f"Successfully booked appointment!\n" f"Location: {city}\n" f"Date: {date}"
    ...
    def run(self):
        ...
        try:
            ...
            self.notification.notify_start_monitoring(
                self.config["appointment"]["preferred_cities"], self.config["appointment"]["date_range"], mode_str
            )
            ...
            while True:
                ...
                logging.info(f"Waiting {self.config['monitoring']['check_interval']} seconds before next check...")
                time.sleep(self.config["monitoring"]["check_interval"])
